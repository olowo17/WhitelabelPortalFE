...
import { resetPin, searchCustomer } from './PinReset.service';
import { IPinCustomer, ISearchPin, ISearchPinForm } from './PinReset.model';

const defaultCustomer: IPinCustomer = {
  customerName: '',
...
const PinReset = () => {
  ...
  const { values, updateValue, updateValueByName, errors, touched, setTouched } = useFormValues<ISearchPinForm>(
    {},
    schema
  );
  const [customer, setCustomer] = useState<IPinCustomer>(defaultCustomer);
  ...
  const onResetSubmit = useCallback(
    (evt) => {
      ...
      setResetting(true);
      resetPin({ accountNumber: customer.primaryAccountNumber })
        .then((data) => {
              ...
              <form style={{ width: '100%' }} onSubmit={onSearchSubmit}>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={4}>
                    <CustomerSelectInput<ISearchPinForm>
                      values={values}
